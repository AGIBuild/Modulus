...
Modulus 提供了统一的 Nuke 构建系统来简化插件的开发和打包流程。
...
nuke plugin --op all
打包特定插件：
# 打包特定插件，例如 SimplePlugin
nuke plugin --op single --name SimplePlugin
#### 打包输出

打包后的插件将输出到以下位置：

- **插件目录**：`artifacts/plugins/{插件名称}/`
- **插件压缩包**：`artifacts/plugins/{插件名称}.zip`

打包过程会显示彩色的构建汇总信息，包括成功和失败的插件列表。打包过程是相互独立的，一个插件的失败不会影响其他插件的打包。如果插件编译或打包失败，相关的输出文件会被自动清理。

#### 开发自定义插件

1. 创建插件项目：dotnet new modulus-plugin -n MyCustomPlugin
2. 开发你的插件功能

3. 打包你的插件：nuke pack
4. 将打包的插件安装到 Modulus：
   - 将插件复制到 `%USERPROFILE%\.modulus\plugins\` 目录（Windows）
   - 或复制到 `~/.modulus/plugins/` 目录（macOS/Linux）

### 手动打包插件

如果你不使用 Nuke 构建系统，也可以手动打包插件：

1. 构建插件项目：dotnet build -c Release
2. 将输出文件（DLL、配置文件、资源文件等）复制到一个新目录
...
7. **使用统一构建系统**：尽可能使用 Nuke 构建系统来开发和打包插件，确保一致性
8. **测试在示例插件上**：开发新功能时，先在示例插件上测试，再集成到自己的插件中
